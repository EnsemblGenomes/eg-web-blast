...
#  my ($prefix, $species, $hit_name) = split /\:/, $hit->seq_id, 3;
  my ($prefix, $hit_name) = split /\:/, $hit->seq_id, 2;


#  warn Dumper $DBAdaptor;

#  warn Dumper $hit;


  # SK 19-Mar-2009. trim 'EG:' prefixed to Fasta display_id - this was
  # added at the request of the EBI webservice. Needs be removed or
  # HSP won't map
#  $hit_name =~s/^EG://;
#warn "S $hit_name * $species";  
...
#STEPHEN TEMP HACK FOR DEV- REMOVE BEFORE PRODCTION......
#STEPHEN TEMP HACK FOR DEV- REMOVE BEFORE PRODCTION......
    my $coord_system_name = $dbseq_meta{subtype} || 'toplevel';

#force coord_system_name to toplevel
    ...
    if( $thingy && $thingy->isa('Bio::EnsEMBL::Feature') ){ # Setter
     #  $self->{genomicNEW} ++; # Flag as new-style. Remove after 23.1 release
      ...
      foreach my $CoordSystem( @{$CSAdaptor->fetch_all} ){
        ...
        my $csFeature;
        eval{
          $csFeature = $Feature->transform($cs);
        };
        if($@ || !$csFeature){next;}
